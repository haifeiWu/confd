...
func NewNacosClient(nodes []string, group string, config constant.ClientConfig) (client *Client, err error) {
	...
	log.Info(fmt.Sprintf("endpoint=%s, namespace=%s, group=%s, accessKey=%s, secretKey=%s, openKMS=%v, regionId=%s", config.Endpoint, config.NamespaceId, group, config.AccessKey, config.SecretKey, config.OpenKMS, config.RegionId))
...
func (client *Client) GetValues(keys []string) (map[string]string, error) {
	...
	for _, key := range keys {
		...
		if strings.HasPrefix(k, "naming.") {
			...
			log.Info(fmt.Sprintf("key=%s, value=%v", key, instances))
			if err == nil {
